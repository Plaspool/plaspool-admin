 ...
 * route the nav also names, including the two — Delivery areas and Where we
 * collect — which deliberately point INTO other sections rather than at
...
describe('the settings index', () => {
  it('links each door to its route, delivery areas included', async () => {
    ...
    expect(hrefOf(/Notifications/)).toBe('/settings/notifications');
    expect(hrefOf(/Team/)).toBe('/settings/team');
    expect(hrefOf(/Writing/)).toBe('/settings/writing');
    // Delivery areas opens the EXISTING screen under Orders — one room, two doors.
    expect(hrefOf(/Delivery areas/)).toBe('/orders/delivery');
    // Where we collect does the same for the Spools section's map of districts.
    ...
    expect(screen.getByText('Who gets an email when an order comes in.')).toBeTruthy();
