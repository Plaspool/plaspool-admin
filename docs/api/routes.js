...
// …or, equivalently, the already-previewed form:
{
  ...
  /* ── SHOP — Orders ──────────────────────────────────────────────────── */
  {
    ...
    desc:
      '<strong>⚠️ This route answers 401 to every caller today, including a customer holding a valid <code>__Host-shop_session</code>.</strong> It is not a bug in the route; it is a seam that nothing has been plugged into. Orders reaches the customer identity through a port (<code>server/shop/orders/ports.ts</code>), and the only registration the composed app makes is <code>registerOrdersDefaults({ mailer })</code> in <code>server/index.ts</code> — no <code>customer</code> resolver. <code>resolveDeps</code> therefore falls back to <code>NO_CUSTOMER</code>, which resolves nobody, and the handler\'s first act is <code>if (!customer) throw new UnauthenticatedError()</code>. Only <code>server/shop/orders/test/app.ts</code> registers a real resolver, which is why the suite is green while the deployed route is not usable.<br><br><strong>The 401 is the honest answer rather than the fallback.</strong> Resolving the writer session instead would make a shop owner\'s own orders list return <em>every</em> customer\'s orders under a URL a storefront calls — so this fails closed on purpose until Cart registers the resolver. The two routes below are unaffected: they authorize through a signed guest token as well, which needs no port.<br><br>Once wired: customer-scoped in SQL, not in application code. The customer projection strips <code>checkoutId</code> and <code>paymentIntentId</code>.',
    res: {
      200: `{
  ...
  },
  {
    ...
    desc:
      'One request for the whole support view: lines, fulfilments, timeline, the emails that were queued or sent, and the payment status pulled through the payment port.<br><br><strong>⚠️ <code>payment</code> is <code>null</code> on every order today.</strong> Like the customer resolver on <code>GET /api/shop/orders</code>, it comes through a port that nothing registers: <code>server/index.ts</code> calls <code>registerOrdersDefaults({ mailer })</code> only, so <code>resolveDeps</code> falls back to <code>payments: null</code> and the field short-circuits to <code>null</code> before <code>paymentIntentId</code> is even consulted. Read it as "not wired yet", <em>not</em> as "this order was never paid" — <code>order.status</code> and <code>order.paidAt</code> are the fields that actually answer that, and they are populated. Admin payment state is available meanwhile on the owner-only <code>GET /api/shop/admin/payments/intents/:id</code>, which reads Payments directly rather than through the port.<br><br>The port is read-only by design: contract §5 gives Orders no way to <em>change</em> payment state, only to display it.',
    res: {
      200: `{
  "order": { … full row … },
  "lines": [ … ],
