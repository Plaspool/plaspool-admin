...
// …or, equivalently, the already-previewed form:
{
  ...
  },
  {
    ...
    group: 'Customer identity',
    m: 'POST',
    p: '/api/shop/customer/session/exchange',
    auth: 'public',
    src: 'server/shop/cart/routes/customer.ts:63',
    sum: 'Exchange a Neon Auth assertion for a customer session.',
    desc:
      'Neon Auth owns sign-in; the storefront mints a short-lived, single-use, HMAC-signed assertion naming a verified email, and this is the only way to become a signed-in customer. A bad, forged, expired, or already-spent assertion is a <strong>400</strong>, not a 401 — the caller has no session to be unauthenticated with, and the fix is a fresh assertion rather than credentials. A replay answers exactly as a forgery does, deliberately, so a captured assertion tells an attacker nothing about whether it was ever real; an expired one is distinct, because that the client can act on.',
    body: `{ "assertion": "…" }   // 1..4096`,
    res: { 200: `{ "customer": { "id": "…", "email": "…", "displayName": "…", "createdAt": 0 } }` },
    sets: ['<code>__Host-shop_session</code>'],
    rl: '<code>shop-exchange:&lt;ip&gt;</code> at 20 / 15 min.',
    errs: [
      [400, 'bad_request', '<code>detail: "assertion" | "assertion_expired"</code>'],
      [403, 'forbidden', ''],
      [429, 'rate_limited', ''],
      [501, 'not_implemented', '<code>{ error, feature: "identity-bridge" }</code> — the bridge secret is not configured on this deployment'],
    ],
  },
  {
    surface: 'shop',
